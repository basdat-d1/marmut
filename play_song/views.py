...
@connectdb
def play_song(cursor, request, id_konten):
    if request.method == 'POST':
        ...
        if progress > 70:
            ...
            """, [str(id_konten)])
    cursor.execute("""
        SELECT k.judul, array_agg(g.genre) AS genres, ak.nama AS artist,
               array_agg(ak2.nama) AS songwriters, k.durasi, k.tanggal_rilis,
    ...
    context = {
        ...
        'is_premium': request.session.get('is_premium', 'False') == 'True',
    }
    return render(request, 'play_song.html', context)

@connectdb
def tambah_lagu_play_song(cursor, request, id_konten):
    ...
    if request.method == 'POST':
        ...
        result = cursor.fetchone()
        if result is None:
            messages.error(request, "Konten tidak ditemukan.")
            return redirect('play_song:play_song', id_konten=id_konten)
        judul_lagu = result[0]
...
@connectdb
def download_song(cursor, request, id_konten):
    email = request.session.get('email')
    if request.method == 'POST':
        id_user_playlist = request.POST.get('playlist')
